...
export default function HostDashboard({ defaultTab = 'listings' }) {
  const { user } = useAuth();
  ...
  const [isProcessingBookingId, setIsProcessingBookingId] = useState(null);
  ...
  const [isResubmittingId, setIsResubmittingId] = useState(null);

  // Availability state
  const [editingAvailabilityListing, setEditingAvailabilityListing] = useState(null);
  ...
  const [isTogglingAvailabilityId, setIsTogglingAvailabilityId] = useState(null);

  const formatAvailabilitySummary = (avail) => {
  ...
  const handleOpenAvailability = (listing) => {
  ...
  const handleSaveAvailability = async (e) => {
    e.preventDefault();
    if (!editingAvailabilityListing) return;
    ...
    setAvailabilityError(null);
    try {
      const updatedAvail = await apiClient.updateAvailability(
  ...
  const handlePauseAvailability = async (listingId) => {
    setIsTogglingAvailabilityId(listingId);
    try {
      const res = await apiClient.pauseAvailability(listingId);
      const newAvail = res.body?.availability || res.availability || {
        ...(listings.find((l) => l.listingId === listingId)?.availability || {}),
        temporarilyUnavailable: true
      };
      setListings((prev) =>
        prev.map((l) =>
          l.listingId === listingId ? { ...l, availability: newAvail } : l
        )
      );
    } catch (err) {
      alert(`Failed to pause availability: ${err.message}`);
    } finally {
      setIsTogglingAvailabilityId(null);
    }
  };

  const handleResumeAvailability = async (listingId) => {
    setIsTogglingAvailabilityId(listingId);
    try {
      const res = await apiClient.resumeAvailability(listingId);
      const newAvail = res.body?.availability || res.availability || {
        ...(listings.find((l) => l.listingId === listingId)?.availability || {}),
        temporarilyUnavailable: false
      };
      ...
      alert(`Failed to resume availability: ${err.message}`);
    } finally {
      setIsTogglingAvailabilityId(null);
    }
  };

  const handleResubmitListing = async (listingId) => {
    setIsResubmittingId(listingId);
    try {
      const updated = await apiClient.resubmitListing(listingId);
  ...
  const fetchListings = async () => {
    ...
    setError(null);
    try {
      const data = await apiClient.getHostListings();
      setListings(data || []);
    } catch (err) {
      setError(err.message || 'Failed to load your parking listings.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      const data = await apiClient.getHostBookings(filters);
      setBookings(data || []);
    } catch (err) {
  ...
  // Reset and load data when authenticated user changes or mounts
  useEffect(() => {
    // Clear stale state from previous user session
    setListings([]);
    setBookings([]);
    setHostProfile(null);
    setError(null);
  ...
  useEffect(() => {
    ...
    if (tabFromUrl && (tabFromUrl === 'listings' || tabFromUrl === 'bookings')) {
  ...
  }, [activeTab, bookingStatusFilter, user?.id]);
  ...
  const formatBookingTimeOnly = (isoString) => {
    if (!isoString) return '';
    try {
      return new Intl.DateTimeFormat('en-US', {
        timeZone: 'Asia/Kolkata',
  ...
  const handleCancelBooking = async (bookingId) => {
    ...
    try {
      const updated = await apiClient.cancelBooking(bookingId);
  ...
  const handleCompleteBooking = async (bookingId) => {
    setIsProcessingBookingId(bookingId);
    try {
      const updated = await apiClient.completeBooking(bookingId);
      setBookings((prev) => prev.map((b) => (b.bookingId === bookingId ? updated : b)));
      if (viewingBooking?.bookingId === bookingId) {
        setViewingBooking(updated);
      }
  ...
  const handleStartSession = async (bookingId) => {
    setIsProcessingBookingId(bookingId);
    try {
      const updated = await apiClient.startSession(bookingId);
      setBookings((prev) => prev.map((b) => (b.bookingId === bookingId ? updated : b)));
      if (viewingBooking?.bookingId === bookingId) {
        setViewingBooking(updated);
      }
    } catch (err) {
      alert(`Failed to start parking session: ${err.message}`);
    } finally {
      setIsProcessingBookingId(null);
    }
  };
  ...
  const handleSaveEdit = async (e) => {
    ...
    try {
      ...
      const updated = await apiClient.updateListing(editingListing.listingId, updates);
    ...
    <section className="dashboard-container" aria-labelledby="dashboard-title">
      ...
      <div className="dashboard-header-row">
        <div>
          ...
          <div className="host-account-status-badge" style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginTop: '0.5rem', flexWrap: 'wrap' }}>
            {hostProfile?.verification?.status === 'verified' || user?.verificationStatus === 'verified' ? (
              <span className="status-pill status-approved" id="host-account-verified-badge" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.25rem', backgroundColor: '#00875A', color: '#fff', padding: '0.25rem 0.6rem', borderRadius: '12px', fontSize: '0.78rem', fontWeight: 600 }}>
                ✓ Verified Host Account
              </span>
            ) : hostProfile?.verification?.status === 'pending' || user?.verificationStatus === 'pending' ? (
              <span className="status-pill status-pending" id="host-account-pending-badge" style={{ padding: '0.25rem 0.6rem', borderRadius: '12px', fontSize: '0.78rem' }}>
                Verification Pending
              </span>
            ) : (
              <span className="status-pill" id="host-account-unverified-badge" style={{ backgroundColor: '#DFE1E6', color: '#42526E', padding: '0.25rem 0.6rem', borderRadius: '12px', fontSize: '0.78rem', fontWeight: 500 }}>
                Verification Required
              </span>
            )}
            <span style={{ fontSize: '0.82rem', color: '#6B778C' }}>
              {user?.email}
            </span>
          </div>
        </div>
        ...
        <>
          ...
          {isLoading && (
            <div className="auth-loading-container" aria-live="polite">
              ...
              <p className="loading-text">Loading your parking listings...</p>
            </div>
          )}
      ...
      {!isLoading && !error && listings.length === 0 && (
        <div className="dashboard-empty-card">
          ...
          <Link to="/host/new" id="btn-empty-list" className="btn btn-primary btn-large">
              ...
              <article key={item.listingId} className="dashboard-listing-card" id={`card-${item.listingId}`}>
                ...
                <div className="listing-card-body">
                  <div className="listing-card-header">
                    ...
                    <span className="listing-price">&#8377;{item.pricing?.hourly}<span className="price-unit">/hr</span></span>
                  </div>
                  ...
                  {item.status === 'pending_review' && (
                    <div className="listing-status-banner status-banner-pending" role="status">
                      ...
                      <p style={{ margin: 0, fontSize: '0.775rem' }}>Your space listing is under administrative review.</p>
                    </div>
                  )}
                  ...
                  {item.status === 'rejected' && (
                    <div className="listing-status-banner status-banner-rejected" role="alert">
                      ...
                      <button
                        ...
                        onClick={() => handleResubmitListing(item.listingId)}
                    ...
                    <div className="availability-btn-row">
                      ...
                      {item.availability?.temporarilyUnavailable ? (
                        <button
                          ...
                          onClick={() => handleResumeAvailability(item.listingId)}
                        ...
                        <button
                          ...
                          className="btn-availability-toggle"
                          onClick={() => handlePauseAvailability(item.listingId)}
                          disabled={isTogglingAvailabilityId === item.listingId}
                        >
                  ...
                  <div className="listing-card-actions">
                    ...
                    <button
                      ...
                      onClick={() => setDeletingId(item.listingId)}
          ...
          {!isLoadingBookings && !bookingsError && bookings.length > 0 && (
            <div className="dashboard-bookings-grid" role="region" aria-label="Host parking reservations">
              {bookings.map((booking) => {
                ...
                const isSessionActive = booking.status === 'confirmed' && !!booking.session?.startedAt && !booking.session?.completedAt;
                    ...
                    <div className="booking-card-body">
                      {isSessionActive && booking.session?.startedAt && (
                        <div className="booking-session-live-alert">
                          ...
                          <span className="live-started-time">Started {formatBookingTimeOnly(booking.session.startedAt)}</span>
                        </div>
                      )}
                      ...
                      <div className="booking-info-row">
                        ...
                        <span className="booking-value">{booking.durationMinutes} mins ({Math.round((booking.durationMinutes / 60) * 10) / 10} hrs)</span>
                      </div>
                      <div className="booking-info-row">
                      ...
                      <div className="booking-info-row">
                        ...
                        <span className="booking-price-value">&#8377;{booking.pricing?.totalAmount} (&#8377;{booking.pricing?.hourlyRate}/hr)</span>
                      </div>
      ...
      {/* VIEW MODAL (Displays full details including owner's private address) */}
      {viewingListing && (
            ...
            <div className="modal-body">
              ...
              <div className="review-content-grid">
                ...
                <div className="review-item">
                  ...
                  <span className="review-item-value">{viewingListing.location?.locality}, {viewingListing.location?.city}</span>
                </div>
                <div className="review-item">
                  <span className="review-item-label">State & PIN Code</span>
                  <span className="review-item-value">{viewingListing.location?.state} - {viewingListing.location?.pincode}</span>
                </div>
                <div className="review-item">
                  <span className="review-item-label">Rate</span>
                  <span className="review-item-value price-highlight">&#8377;{viewingListing.pricing?.hourly} / hr</span>
                </div>
                ...
                <div className="review-item">
                  ...
                  <span className="review-item-value">{viewingListing.parkingDetails?.vehicleType} ({viewingListing.parkingDetails?.parkingType})</span>
                </div>
                <div className="review-item">
              ...
              <div className="modal-body">
                ...
                <div className="form-group">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '0.4rem' }}>
                    <label className="form-label" style={{ margin: 0 }}>Operating Days <span className="required-star">*</span></label>
                ...
                <div className="form-group">
                  ...
                  <span style={{ fontSize: '0.75rem', color: 'var(--color-text-muted)', marginTop: '0.25rem', display: 'block' }}>
                    Standard operating timezone for ParkSync India network.
                  </span>
                </div>

                <div className="form-group" style={{ backgroundColor: 'var(--color-surface-subtle)', padding: '0.85rem', borderRadius: 'var(--radius-sm)', marginTop: '0.5rem' }}>
      ...
      {/* =================================================================== */}
      {viewingBooking && (() => {
        const isSessionActiveModal = viewingBooking.status === 'confirmed' && !!viewingBooking.session?.startedAt && !viewingBooking.session?.completedAt;
            ...
            <div className="modal-card" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="modal-body">
                ...
                <div className="booking-timeline-container" aria-label="Booking lifecycle timeline">
                  ...
                  <div className={`timeline-step ${viewingBooking.session?.startedAt ? (viewingBooking.status === 'completed' ? 'step-completed' : 'step-active') : ''}`}>
                    <span className="timeline-circle">
                ...
                {(viewingBooking.access?.instructions || viewingBooking.access?.entryNotes || viewingBooking.access?.hostInstructions) && (
                ...
                <div className="modal-detail-grid">
                  ...
                  <div className="modal-detail-item">
                    ...
                    <span className="detail-value">
                      {formatBookingDate(viewingBooking.startAt)}
                    </span>
                  </div>
                  ...
                  <div className="modal-detail-item">
                    ...
                    <span className="detail-value">
                      &#8377;{viewingBooking.pricing?.hourlyRate}/hr
                    </span>
                  </div>
                  ...
                  <div className="modal-detail-item">
                    ...
                    <span className="detail-value">
                      {formatBookingDateTime(viewingBooking.createdAt)}
                    </span>
                  </div>
                  ...
                  {viewingBooking.completedAt && !viewingBooking.session?.completedAt && (
                    <div className="modal-detail-item">
                      ...
                      <span className="detail-value">
                        {formatBookingDateTime(viewingBooking.completedAt)}
                      </span>
                    </div>
                  )}
                  ...
                  {viewingBooking.cancelledAt && (
                    <div className="modal-detail-item">
                      ...
                      <span className="detail-value">
                        {formatBookingDateTime(viewingBooking.cancelledAt)}
                      </span>
                    </div>
                  )}
                </div>
